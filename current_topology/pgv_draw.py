...
def pgv_diagram(
    root_info, nei_list, save_dir=os.getcwd(), save_subdir="test_output"
):
    ...
    # rnode.attr['bgcolor'] = "grey"
    # rnode.attr['font color'] = "white"
    rnode.attr["color"] = "#ffffff"

    # Iterate over all the CDP neighbors
    for nei in nei_list:

        # Create and add a node for each destination host from the CDP Neighbors data
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Script Description", epilog="Usage: ' python pgv_draw' "
    )
